...
estimateSNR=function(x,numPerm=1000,nu=NA){
  ...
  if(is.na(nu) || nu<0 ){
    ...
    if(!class(myfit)=="mle.ghyp"){
      ...
      #    list(nu=NA,SNR=NA,R0bar=NA,N=length(x))   # could not fit at all
    }else{
      nu=coef(myfit)$nu
    }
  }
  ...
  SNR_raw=theta(abs(R0bar)/length(x),nu)                #splines have been calibrated for positive SNRs, hence abs(R0bar)
